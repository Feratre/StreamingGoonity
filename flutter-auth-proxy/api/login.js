...
module.exports = async (req, res) => {
  return cors(req, res, async () => {
    ...
    try {
      ...
      // Poi effettua la richiesta di login con i cookie impostati
      const response = await axiosInstance.post(
        'http://redaproject.page.gd/login.php',
        { email, password },
        { 
          headers: { 
      ...
      if (typeof response.data === 'string') {
        if (response.data.includes('success') && response.data.includes('true')) {
          // Cerca di estrarre i dati dell'utente
          try {
            ...
            if (jsonMatches && jsonMatches.length > 0) {
              ...
              // Fallback per login riuscito ma senza dati utente
              result = { 
                success: true, 
        ...
        } else if (response.data.includes('non trovato') || response.data.includes('non corretta')) {
          result = { success: false, message: 'Email o password non corretti' };
        } else {
      ...
      console.error('Errore proxy login:', error);
