...
class CatalogItem {
  ...
  factory CatalogItem.fromTmdbJson(
    Map<String, dynamic> json,
    String mediaType,
    List<Map<int, String>> genreMaps,
  ) {
    ...
    try {
      final response = await http.post(
        ...
        headers: {
          ...
          'User-Agent':
              'Mozilla/5.0 Flutter App', // Simula un browser per evitare blocchi
...
class PlaybackPositionManager {
  ...
  static Future<Map<String, dynamic>?> getLastWatchedEpisode(
    String mediaType,
    int id,
  ) async {
...
class _StreamingWebViewState extends State<StreamingWebView> {
  ...
  int retryCount = 0; // Contatore per i tentativi di ricaricamento
  static const int maxRetries = 3; // Numero massimo di tentativi
  ...
  void initState() {
    ...
    // Configurazioni specifiche per Android WebView
    ...
    _controller = WebViewController()
      ...
      // User-Agent più recente e realistico (Chrome 131)
      ..setUserAgent(
    ...
    // Configurazioni specifiche Android per migliorare compatibilità
      ...
      ..setNavigationDelegate(
        NavigationDelegate(
          onPageStarted: (String url) {
            ...
            // Timeout aumentato a 60 secondi per connessioni lente
            Future.delayed(Duration(seconds: 60), () {
              if (isLoading && mounted) {
                setState(() {
                  ...
                  hasError = true;
                  errorMessage =
                      "Timeout di caricamento (60s). Il server potrebbe essere irraggiungibile o sovraccarico.";
                });
              }
            });
          },
          ...
          onWebResourceError: (WebResourceError error) {
            ...
            // Tenta di ricaricare con backoff esponenziale
            // Errori comuni: -1 (generic), -2 (timeout), -8 (connection timeout)
            if ((error.errorCode == -1 ||
              ...
              final retryDelay = Duration(
                seconds: 3 * retryCount,
              ); // Backoff esponenziale: 3s, 6s, 9s

              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(
                      "Tentativo ${retryCount}/$maxRetries di ricaricamento in ${retryDelay.inSeconds}s...",
                    ),
                    duration: retryDelay,
                  ),
                );
              ...
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text(
                      "Impossibile caricare la pagina dopo 3 tentativi. Verifica la connessione o prova più tardi.",
                    ),
  ...
  void dispose() {
    ...
    // Log per debugging
    print(
      "StreamingWebView dispose - saving final position: $playbackPosition",
    );
  ...
  Future<void> _savePlaybackPosition({bool showNotification = true}) async {
    if (playbackPosition != null && playbackPosition! > 0) {
      ...
      if (AuthService.isLoggedIn && AuthService.userId != null) {
        ...
        print(
          "Saving to database - UserID: ${AuthService.userId}, Content: ${widget.title}, Position: $playbackPosition",
        );
        ...
        final success = await WatchedContentService.saveWatchedContent(
          watchedContent,
        );
      ...
      if (showNotification && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Posizione salvata: ${PlaybackPositionManager.formatTime(playbackPosition!)}',
            ),
...
class CatalogService {
  ...
  List<CatalogItem> getItems(String type) =>
      type == 'movie' ? _movieItems : _tvItems;
  List<int> getTmdbIds(String type) =>
      type == 'movie' ? _movieTmdbIds : _tvTmdbIds;
  bool isLoading(String type) =>
      type == 'movie' ? _isLoadingMovie : _isLoadingTv;
  bool isLoadingMore(String type) =>
      type == 'movie' ? _isLoadingMoreMovie : _isLoadingMoreTv;
  String getError(String type) => type == 'movie' ? _movieError : _tvError;
  String getRawResponse(String type) =>
      type == 'movie' ? _movieRawResponse : _tvRawResponse;
  int getLoadedItemsCount(String type) =>
      type == 'movie' ? _loadedMovieItemsCount : _loadedTvItemsCount;
  int getTotalItems(String type) =>
      type == 'movie' ? _totalMovieItems : _totalTvItems;
  bool hasInitialized(String type) =>
      type == 'movie' ? _hasInitializedMovie : _hasInitializedTv;
  ...
  Future<void> loadGenres() async {
    ...
    try {
      ...
      final movieGenresUrl =
          'https://api.themoviedb.org/3/genre/movie/list?api_key=$apiKey&language=it';
      ...
      final tvGenresUrl =
          'https://api.themoviedb.org/3/genre/tv/list?api_key=$apiKey&language=it';
  ...
  Future<void> searchInCatalog(
    ...
    try {
      ...
      final url =
          'https://api.themoviedb.org/3/search/$searchType?api_key=$apiKey&query=$query&language=it';
  ...
  Future<void> fetchCatalogIds(String type, Function setState) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final initialBatchSize = tmdbIds.length < initialLoadCount
            ? tmdbIds.length
            : initialLoadCount;
        final initialBatch = tmdbIds.sublist(0, initialBatchSize);

        // Crea placeholder per i primi elementi
        List<CatalogItem> initialPlaceholders = initialBatch
            .map(
              (id) =>
                  CatalogItem(id: id, title: 'Caricamento...', mediaType: type),
            )
            .toList();
  ...
  Future<void> loadMoreTitles(String type, Function setState) async {
    if (isLoadingMore(type) ||
        getLoadedItemsCount(type) >= getTmdbIds(type).length) {
      return;
    }
    ...
    final end =
        (getLoadedItemsCount(type) + loadMoreCount <= getTmdbIds(type).length)
  ...
  Future<void> fetchTitlesForIds(
    ...
    for (int i = 0; i < ids.length; i += maxConcurrentRequests) {
      final end = (i + maxConcurrentRequests < ids.length)
      ...
      final results = await Future.wait(
        chunk.map((id) async {
          try {
            final tmdbUrl =
                'https://api.themoviedb.org/3/$tmdbType/$id?api_key=$apiKey&language=it';
            ...
            if (tmdbResponse.statusCode == 200) {
              ...
              final item = CatalogItem.fromTmdbJson(
                tmdbData,
                tmdbType,
                genreMaps,
              );
          ...
          return CatalogItem(id: id, title: 'ID: $id', mediaType: tmdbType);
        }),
      );
  ...
  static Future<bool> saveItem(CatalogItem item) async {
    try {
      ...
      if (!savedItems.any((savedItem) {
        ...
        return savedItemMap['id'] == item.id &&
            savedItemMap['mediaType'] == item.mediaType;
      })) {
  ...
  static Future<bool> removeItem(int id, String mediaType) async {
    try {
      ...
      savedItems.removeWhere((savedItem) {
        Map<String, dynamic> savedItemMap = json.decode(savedItem);
        return savedItemMap['id'] == id &&
            savedItemMap['mediaType'] == mediaType;
      });
...
class WatchedContentService {
  ...
  static Future<bool> saveWatchedContent(WatchedContent content) async {
    ...
    try {
      print(
        "Preparing to save watched content to ${AuthService.baseUrl}$_endpoint",
      );
      final jsonData = {'action': 'save', 'content': content.toJson()};
  ...
  static Future<List<WatchedContent>> getWatchedContent() async {
    ...
    try {
      final response = await http.post(
        Uri.parse('${AuthService.baseUrl}$_endpoint'),
        headers: {'Content-Type': 'application/json'},
        body: json.encode({'action': 'get', 'user_id': AuthService.userId}),
      );
  ...
  Future<void> search() async {
    ...
    if (choice == 'film') {
      ...
      final response = await http.get(
        Uri.parse("$urlF?api_key=$apiKey&query=$titolo"),
      );
      final data = json.decode(response.body);
      if (data["results"] != null && data["results"].isNotEmpty) {
      ...
      final response = await http.get(
        Uri.parse("$urlS?api_key=$apiKey&query=$titolo"),
      );
      final data = json.decode(response.body);
      if (data["results"] != null && data["results"].isNotEmpty) {
        final tmdbId = data["results"][0]["id"];
  ...
  Future<void> openInBrowser(String url) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text("Non posso aprire il link: $url")));
    }
  }
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => StreamingWebView(
          ...
          seasonNumber: choice == 'serie'
              ? int.tryParse(seasonController.text)
              : null,
          episodeNumber: choice == 'serie'
              ? int.tryParse(episodeController.text)
              : null,
        ),
      ),
    );
            ...
            children: [
              Expanded(
                child: Text(
                  ...
                  style: const TextStyle(color: Color(0xFF5E72E4)),
                ),
              ),
              IconButton(
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (SearchService.pendingSearchQuery != null &&
          SearchService.pendingSearchType != null) {
          ...
          Padding(
            ...
            child: Container(
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  hintText:
                      'Cerca ${_selectedType == 'movie' ? 'film' : 'serie TV'}...',
    ...
    if (_catalogService.isLoading(type)) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            const CircularProgressIndicator(color: Color(0xFF5E72E4)),
            const SizedBox(height: 20),
            Text(
...
class _SavedContentPageState extends State<SavedContentPage> {
  ...
  void _openDetailPage(CatalogItem item) {
    Navigator.push(
      ...
      (_) => _loadSavedItems(),
    ); // Ricarica gli elementi quando torni indietro
  }
                      ...
                      subtitle: Column(
                        ...
                        children: [
                          ...
                          if (item.releaseDate != null &&
                            ...
                            Text(
                              "Anno: ${item.releaseDate!.length >= 4 ? item.releaseDate!.substring(0, 4) : item.releaseDate!}",
                              style: TextStyle(color: Colors.grey[400]),
                            ),
                        ],
                      ),
...
class _DetailPageState extends State<DetailPage>
  ...
  Future<void> _loadMovieDetails() async {
    try {
      final url =
          'https://api.themoviedb.org/3/movie/${widget.item.id}?api_key=$apiKey&language=it';
      final response = await http.get(Uri.parse(url));
  ...
  Future<void> _checkIfSaved() async {
    final saved = await SavedContentManager.isItemSaved(
      widget.item.id,
      widget.item.mediaType,
    );
    if (mounted) {
  ...
  Future<void> _toggleSave() async {
    if (isSaved) {
      ...
      final success = await SavedContentManager.removeItem(
        widget.item.id,
        widget.item.mediaType,
      );
      if (success && mounted) {
  ...
  Future<void> _loadSeasons() async {
    ...
    try {
      final url =
          'https://api.themoviedb.org/3/tv/${widget.item.id}?api_key=$apiKey&language=it';
      final response = await http.get(Uri.parse(url));
  ...
  void openInWebView(
    ...
    bool isAvailable = await AvailabilityService.isContentAvailable(
      mediaType,
      id,
    );
    if (!isAvailable) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              "Questo contenuto non è disponibile per lo streaming",
            ),
          ),
        );
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: widget.item.mediaType == 'tv'
          ? TabBarView(
              controller: _tabController,
              children: [_buildInfoTab(), _buildSeasonsTab()],
            )
          : _buildInfoTab(),
    );
  }
                ...
                if (widget.item.posterPath != null)
                  ShaderMask(
                    ...
                    child: Image.network(
                      ...
                      fit: BoxFit.cover,
                      errorBuilder: (context, error, stackTrace) =>
                          Container(color: const Color(0xFF1F2133)),
                    ),
                  ),
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            Container(
                              ...
                              child: Hero(
                                tag:
                                    'poster_${widget.item.id}_${widget.item.mediaType}',
                                child: ClipRRect(
                                  borderRadius: BorderRadius.circular(12),
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  Hero(
                                    tag:
                                        'title_${widget.item.id}_${widget.item.mediaType}',
                                    child: Material(
                                      color: Colors.transparent,
                                      child: Text(
                                        widget.item.title,
                                  ...
                                  if (widget.item.releaseDate != null &&
                                      widget.item.releaseDate!.isNotEmpty)
                                    Container(
                                      padding: const EdgeInsets.symmetric(
                                        horizontal: 10,
                                        vertical: 4,
        ...
        child: InkWell(
          ...
          onTap: () {
            if (widget.item.mediaType == 'movie') {
              ...
              PlaybackPositionManager.getLastWatchedEpisode(
                widget.item.mediaType,
                widget.item.id,
              ).then((lastEpisode) {
                if (lastEpisode != null) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        automaticallyImplyLeading:
            widget.showBackButton, // Usa il parametro qui
      ),
      body: SingleChildScrollView(
        padding: const EdgeInsets.all(24),
        child: Column(
...
class _AuthenticationGateState extends State<_AuthenticationGate> {
  ...
  @override
  Widget build(BuildContext context) {
    return AuthService.isLoggedIn ? widget.child : const AuthScreen();
  }
}
